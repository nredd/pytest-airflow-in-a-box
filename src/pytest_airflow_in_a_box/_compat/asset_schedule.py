...
``_evaluate_v3`` is independently authored and adapted from the DagRun-creation body of
Apache Airflow's ``SchedulerJobRunner._create_dag_runs_asset_triggered`` and the
readiness evaluation in ``DagModel.dags_needing_dagruns``. ``_evaluate_v2`` is adapted
analogously from ``SchedulerJobRunner._create_dag_runs_dataset_triggered``. Both drop
scheduler-operational concerns that do not apply to one evaluated test Dag inside an
isolated single-process test database: row locking, ``max_active_runs`` throttling,
paused/stale/import-error Dag filters, and batching. Both also simplify the attached
"consumed events" query to omit the real scheduler's lower bound at the previous
asset/dataset-triggered run of the same consumer, which only matters across repeated
evaluations of the same consumer Dag across multiple producer runs; a single evaluation
per consumer (the common test shape) is unaffected. Aliased asset/dataset consumers
(``DagScheduleAssetAliasReference`` / ``DagScheduleDatasetAliasReference``) are out of
...
def _evaluate_v3_dag(dag_id: str, session: Session) -> DagRun | None:
    ...
    Raises:
        ValueError: No serialized Dag is persisted for ``dag_id``, or the Dag is not
            scheduled by an ``Asset``.
    """

    from airflow.assets.evaluation import AssetEvaluator
    from airflow.models.asset import AssetDagRunQueue, AssetEvent, DagScheduleAssetReference
    from airflow.models.dagrun import DagRun
    ...
    asset_ids = {adrq.asset_id for adrq in queued}
    asset_events = (
        session.scalars(
            select(AssetEvent)
            .join(
    ...
    session.execute(
        delete(AssetDagRunQueue).where(
            AssetDagRunQueue.target_dag_id == dag_id,
            AssetDagRunQueue.asset_id.in_(asset_ids),
        )
    )
    session.flush()
    return dag_run
