...
finalized when the teardown report for that nodeid arrives: a failed teardown or setup
report always wins as `error` (carrying the failing `phase`); otherwise the call report
decides `passed`/`xpassed`/`failed`/`skipped`/`xfailed`; a setup-only skip (no call
report at all) is `skipped`. `pytest_runtest_logreport` is the one hook that uniformly
receives every report regardless of xdist: on an xdist controller it receives reports
forwarded (and reserialized) from workers, so nothing beyond `report.user_properties`
survives the hop -- which is exactly why the `gated` flag is stashed there by a
...
so this module tracks the single active `pytest.Config` for the current process as a
module-level reference set in `configure()`/cleared in `unconfigure()`. That is safe
because a process running this plugin has exactly one active pytest session at a time:
serial runs, and each xdist controller and worker, are all one `Config` per process for
their lifetime; only nested in-process `pytest.main()` calls could rotate it, and
`configure`/`unconfigure` bracket each such session.
...
# The one side-channel `pytest_runtest_logreport` needs; see the module docstring.
...
def configure(config: pytest.Config) -> None:
    """Install the per-session outcome accumulator and remember the active config.
...
def unconfigure(config: pytest.Config) -> None:
    """Forget the active config when its session ends.
