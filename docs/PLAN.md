...
- **The 100 % branch-coverage gate was unenforceable as configured** — the Windows `windll`
  branch is unreachable on every CI platform, subprocess pytester children were unmeasured, and
...
Full research record: the companion research doc (21 sections, all decisions below
...
file goes silent") fixed reactively three separate times before the
...
latent bug on any NFS-hosted repo, which is *most* of the shared build fleet.
...
is independently implemented from public Airflow behavior; private-fork-only source is not copied.
...
- **A — DagBag import + structure assertion.** `tests/unit/dags/<domain>/test_<dag>.py`
  ...
  (in the fork's fixtures module) is the whole-tree, session-scoped version — generalizes to
...
- **B — custom operator subclass → run → assert output.** `create_task_instance_of_operator` +
  `session`, from the fork's operator test suite. Purest no-DB variant is the fork's
  operator-init test: instantiate the subclass, assert default attrs,
...
- **D — inline Dag + run-all-TIs + assert DagRun SUCCESS.** The run-loop idiom repeated
  ...
  the fork's task-archival test (`@pytest.mark.compat` + `db_test` +
...
**Provenance boundary:** no private-fork-only code, hostnames, paths, credentials, tests, or repository
...
**`ruff` policy:** match the lint families currently used in the predecessor repo:
...
the fork's generator script.** `uv run --isolated --with apache-airflow-core==X` resolves cleanly per Airflow
...
**The fork's constraints-to-pinned-extras generator script is explicitly
...
- Real xdist run (`-n auto`) on both a local developer host and a network-homed CI host — the latter is
