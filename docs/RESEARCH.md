...
were measured directly on two target hosts (two representative target hosts)
...
**Scale.** The fork's conftest bootstrap sits in front of
the predecessor's core plugin module (a near-verbatim fork of upstream's ~3,125-line plugin), plus `tests/common/test_utils/*` (mostly file-for-file mirrors of upstream, plus one
fork-only addition).
...
installed and imported successfully in ~10s via `uv run --isolated --with`, no extra generated by that script required.
...
whole suite; cleanest compat-marked exemplar is the fork's task-archival test (`@pytest.mark.compat` + `db_test` + `timeout(200)`), whose `archive`
