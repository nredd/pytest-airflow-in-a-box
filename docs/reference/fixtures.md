...
| `dag_maker` | function | yes | 2.x + 3.x | A factory building and persisting a Dag authored in the test, with `run()` / `run_ti()` execution; accepts upstream `tests_common`'s `session`/`bundle_name`/`bundle_version` harness keywords and exposes the scheduler-side `serialized_dag`/`dag_model`/`sync_dagbag_to_db()` handles ([Task execution](../guide/task-execution.md#scheduler-side-handles)) |
