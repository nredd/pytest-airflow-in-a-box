...
_Canonical research record. Imported from source commit `abd7747` and revised on 2026-08-07 to
...
reading the fork's `tests/conftest.py` (834 lines), `tests/common/pytest_plugin.py` (2,765 lines),
...
A separate research document (`pytest-airflow.pdf`, a "Unit Testing Challenges and Proposals in
...
were measured directly on two target hosts (a 12-CPU WSL2 dev box and a 4-CPU NFS-home CI host)
...
itself**: whether to adopt the fork's `scripts/update_deps.py` helper. Verified `uv run --with
...
--constraint`, not `update_deps.py`) rather than either unconstrained resolution or the fork's
...
**Scale.** `tests/conftest.py` (834 lines, repo-specific bootstrap) sits in front of
`tests/common/pytest_plugin.py` (2,765 lines — a near-verbatim fork of upstream's 3,125-line
plugin), plus `tests/common/test_utils/*` (mostly file-for-file mirrors of upstream, plus one
fork-only addition).

**Novel engineering, not present upstream:**

1. **`dictConfig` monkeypatch reattachment** (`tests/conftest.py:385-405`). Airflow calls
...
2. **Two-tier FileLock xdist coordination with independent completion markers**
   (`tests/conftest.py:745-758`). One `FileLock` guards two *independently gated* concerns — DB
...
6. **`run_task_instance()` compat shim** (`tests/common/test_utils/taskinstance.py:52-121`) for
...
7. **Self-testing discipline.** `tests/unit/core/test_pytest_plugin.py` (746 lines) unit-tests the
...
**Explicit upstreaming intent already on record.** `pyproject.toml:73` in the fork carries
`# TODO(redd): Upstream tests/common into apache-airflow-devel-common` — i.e., you had already
...
(your fork's own `pyproject.toml:73` TODO already flags this exact fork point).
...
Benchmark (300 inserts / 30 commits / 30 selects), WSL2 dev box, 12 CPU / 15 GiB RAM:
...
"postgres", "mysql")` (`pytest_plugin.py:120`) already exist upstream but are **inert in the fork** —
...
(`grep`-counted fixture/import usage in `ws_1/tests/unit`), not by re-deriving from first principles.
...
log file goes silent") was fixed reactively three separate times (2025-07, 2025-08→10, 2026-07)
...
per-Airflow-version pinned extras (`airflow_3_1_8 = [...]`, `airflow_3_2_2 = [...]`, etc.) — those
exist because `infra-airflow_dags` *deploys* pinned providers, which a plugin never does. Testing
...
- The Makefile (~200 lines) is overwhelmingly private Dag-repo deployment tooling
  (`check_ez_configs`, `check_kraken_configs`, `check_regression_configs`, `check_run_areas`,
  `dag_processor`, `standalone`) — irrelevant to a testing plugin.
...
## 20. Multi-Airflow-version compat testing — `update_deps.py` is not needed for v1

Question: does `scripts/update_deps.py` (the fork's 660-line helper that generates pinned
...
installed and imported successfully in ~10s via `uv run --isolated --with`, no
`update_deps.py`-generated extra required.
...
**The fix is Airflow's own published constraints files directly, not `update_deps.py`'s
...
**Why `update_deps.py`'s actual purpose doesn't transfer to this repo.** Reading it (660 lines,
`scripts/update_deps.py` in the fork): its job is generating a **committed, versioned dependency
group** (`airflow_3_1_8 = [...]`) containing ~100 exactly-pinned *provider* packages
...
**Decision: don't adopt `update_deps.py`.** CI matrix cells fetch the matching
...
release-time compat verification rather than routine CI), revisit `update_deps.py`'s core
...
multi-Airflow-version compat testing mechanism (constraints files, not `update_deps.py`).
...
Findings below come from scanning the fork's `tests/unit/` (`ws_0`/`ws_1`, same repo). Three
takeaways up front:
...
```
The `full_dag_bag` fixture (`tests/unit/fixtures.py:52-62`) is the whole-tree version
...
`session`, from `tests/unit/operators/test_bash.py:242-260`:
```python
...
```
Purest no-DB variant: `test_bash.py:32-52` `test_bash_operator_init` — instantiate the subclass,
assert its default attrs, zero fixtures.

**C. Custom TaskFlow `@task` decorator → run → assert XCom** —
`tests/unit/decorators/test_bash.py:82-101` (init) and `:152-174` (XCom via `run_task_instance`):
```python
with dag_maker(dag_id=...) as dag:
    @task
    def my_task(x): return x + 1
    my_task(41)
...
whole suite; cleanest compat-marked exemplar is `tests/unit/core/test_tasks.py:235-286`
`TestArchive.test_archive` (`@pytest.mark.compat` + `db_test` + `timeout(200)`), whose `archive`
task is nearly generic (`shutil`-based). The canonical loop:
```python
dr = dag_maker.create_dagrun()
for ti in dr.get_task_instances(session=session):
    run_task_instance(ti, session=session)
...
```
Use `ordered_task_instances(dr, dag, session=session)` (`taskinstance.py:32`) instead of
...
| 8 | `test_structlog_caplog_capture` | — | `caplog` (structlog override) | Airflow-3 structlog records are visible to the overridden `caplog` (model: `test_ndisk.py:181`, but a benign logger) |
...
No corporate-only code, hostnames, paths, credentials, test data, or repository history are copied.
