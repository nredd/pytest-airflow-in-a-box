...
| Option / ini | Feeds |
| --- | --- |
| `--dag-folder` / `airflow_dags_folder` | The `dag_bag` fixture (and through it [`run_dag`](task-execution.md#testing-a-dag-defined-elsewhere)), the smoke catalog, and `dag_corpus` |
...
[`run_dag(dag, executor=...)`](task-execution.md#executor-driven-runs) re-imports the Dag
