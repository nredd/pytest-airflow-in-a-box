...
- Cross-Dag relations -- your producer's outlet actually triggering your consumer. See
  [Cross-Dag relations](cookbook.md#cross-dag-relations-asset-triggered-downstream-dags)
- DagRun-to-DagRun relations -- `depends_on_past`, backfill-ish sequences, anything where run
  N-1 conditions run N. See
  [DagRun relations](cookbook.md#dagrun-relations-depends-on-past-and-backfill-ish-sequences)
...
Note the "stock" qualifier. `test_dag_serialization_roundtrip` in the
[smoke catalog](smoke-tests.md) is worth running precisely because *your* operator's
constructor arguments are the part that can fail to serialize.
...
Breeze environment and `tests_common`. The line is roughly "a component that lives in your
repo alongside your Dags" versus "a component you are shipping to other people as part of
Airflow".
...
- [Migration outcome diff](migration-diff.md) -- record outcomes on one Airflow, compare
  against another
...
[What a dagbag + callable test misses](cookbook.md#what-a-dagbag-callable-test-misses) is the
complement to this page: this one is about tests that should not exist, that one is about the
seams a dagbag import test plus a direct `task.function` call never reaches.
