...
def test_smoke_catalog_and_full_dag_bag_consumers_share_one_xdist_group(
    ...
    """Group the smoke catalog with `full_dag_bag` consumers, but respect an explicit group.
    ...
    smoke item and every `full_dag_bag` consumer onto the shared
    `FULL_DAG_BAG_XDIST_GROUP`, except for an item that already carries its own
    explicit `xdist_group`, which is left alone.
