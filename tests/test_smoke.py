...
def test_policy_boolean_readers_default_off_and_read_the_ini(reader: Any, configure: Any) -> None:
    """Return disabled by default, honor an explicit enable, and reject a non-boolean."""

    assert reader(_config()) is False
    assert reader(configure(True)) is True
    with pytest.raises(pytest.UsageError, match="must be a boolean"):
        reader(configure("yes"))
...
def test_serialized_dag_cache_applies_sampling(
    ...
    # The "narrowed coverage" announcement is logged by `dagcorpus._select_serialization_sample`
    # (`pytest_airflow_in_a_box.dagcorpus` logger), not `smoke.py`'s own logger -- both need
    # scoping here since `_serialized_dag_cache` delegates sample selection to it.
    with (
        caplog.at_level("INFO", logger="pytest_airflow_in_a_box.smoke"),
        caplog.at_level("INFO", logger="pytest_airflow_in_a_box.dagcorpus"),
    ):
        entries = smoke._serialized_dag_cache(_session(), _config(sample_size="1"))

    assert len(entries) == 1
    assert set(entries) == set(
        dagcorpus._sampled_dag_ids(("alpha", "bravo", "charlie"), sample_size=1, seed="0")
    )
...
def _corpus_dag(dag_id: str, *, catchup: bool, can_be_scheduled: bool) -> dagcorpus.CorpusDag:
    ...
    Returns:
        pytest_airflow_in_a_box.dagcorpus.CorpusDag containing the described Dag.
    """

    return dagcorpus.CorpusDag(
        dag_id=dag_id,
        tags=frozenset(),
        tasks=(),
        can_be_scheduled=can_be_scheduled,
...
def _mapped_task(
    ...
    Returns:
        pytest_airflow_in_a_box.dagcorpus.CorpusTask containing the described task.
    """

    return dagcorpus.CorpusTask(
        task_id=task_id,
        owner="team",
        pool="default_pool",
        is_mapped=mapped,
...
def test_smoke_catalog_reuses_dag_bag_parse(pytester: pytest.Pytester) -> None:
    """Parse the Dag folder once when a `dag_bag` consumer runs with the catalog.
    ...
    `dag_bag` in the same worker process. The bundled catalog is always collected
    last (`collection.py`), so in this default ordering `dag_bag`'s consumer test
...
""",
    ...
    pytester.makepyfile(
        ...
        def test_consumer(dag_bag):
            assert set(dag_bag.dags) == {"counted_dag"}
        """
    )
...
def test_smoke_integrity_enforces_parse_timeout_after_dag_bag_parses(
    pytester: pytest.Pytester,
) -> None:
    """Honor the configured parse timeout even when `dag_bag` parses first.

    Regression test for issue #85: before the shared-cache fix, a DagBag `dag_bag`
    ...
    pytester.makepyfile(
        ...
        def test_consumer(dag_bag):
            ...
            # under `dag_bag`'s own parse too; only presence is asserted here.
    ...
    # `dag_bag` parsed) from the post-hoc slowpoke/duration check alone, which
...
def test_smoke_disable_drops_every_non_serialization_item(pytester: pytest.Pytester) -> None:
    """Drop two non-serialization core items, keeping the serialization pair and the rest."""
...
def test_smoke_disable_still_validates_the_disabled_checks_own_ini(
    ...
    disabled-check order would silently accept `airflow_pools = malformed` (or any other
    check's bad value) as long as the check naming it never collects.
    """
    ...
    assert result.ret != 0
    result.stderr.fnmatch_lines(["*`airflow_pools` line must be `name = slots`: 'malformed'*"])
...
def test_duplicate_dag_ids_fail_integrity(pytester: pytest.Pytester) -> None:
    """Fail the integrity item and name both colliding files.

    There is no dedicated duplicate-id catalog entry: a duplicate `dag_id` collision
    surfaces as an ordinary `import_errors` entry, which `test_dag_bag_integrity`
    already reports for every file regardless of the failure's cause.
    """
...
def test_markexpr_override_reaches_every_conditional_smoke_item(
    ...
    failure path, and has no catchup or mapped-expand tasks so the two opt-in policy
    checks pass too.
    """
    ...
    pytester.makeini(
        ...
        "airflow_forbid_default_owner = true\n"
        "airflow_dag_snapshot_dir = snapshots\n"
        "airflow_forbid_catchup = true\n"
        "airflow_forbid_unbounded_expand = true\n"
    )
    pytester.makepyfile(
        test_regular="""
        def test_regular():
