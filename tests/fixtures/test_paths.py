"""Test the `airflow_home_path` and `airflow_dags_folder_path` fixtures.
...
def test_airflow_home_path_is_the_run_root(pytester: pytest.Pytester) -> None:
    ...
    pytester.makepyfile(
        ...
        def test_home(airflow_home_path):
            assert isinstance(airflow_home_path, Path)
...
def test_airflow_home_path_needs_no_database(pytester: pytest.Pytester) -> None:
    ...
    pytester.makepyfile(
        ...
        def test_home(airflow_home_path):
            assert airflow_home_path.is_dir()
...
def test_dags_folder_path_falls_back_to_the_bootstrap_scratch_folder(
    ...
    pytester.makepyfile(
        """
        def test_folder(airflow_home_path, airflow_dags_folder_path):
            assert airflow_dags_folder_path == airflow_home_path / "dags"
        """
    )
...
def test_dags_folder_path_follows_the_ini_option(pytester: pytest.Pytester) -> None:
    """Resolve a relative ini value against the pytest root, as `full_dag_bag` does."""
    ...
    pytester.makepyfile(
        """
        def test_folder(pytestconfig, airflow_dags_folder_path):
            assert airflow_dags_folder_path == pytestconfig.rootpath / "dags"
        """
    )
...
def test_dags_folder_path_follows_the_command_line_option(pytester: pytest.Pytester) -> None:
    """Let `--dag-folder` win over the ini option, matching `full_dag_bag`'s ladder."""
    ...
    pytester.makepyfile(
        ...
        def test_folder(airflow_dags_folder_path):
            assert airflow_dags_folder_path == Path({str(chosen)!r})
        """
    )
...
def test_dags_folder_path_is_what_full_dag_bag_parsed(pytester: pytest.Pytester) -> None:
    ...
    pytester.makepyfile(
        """
        def test_folder(full_dag_bag, airflow_dags_folder_path):
            assert "one" in full_dag_bag.dags
            assert (airflow_dags_folder_path / "one.py").is_file()
        """
    )
